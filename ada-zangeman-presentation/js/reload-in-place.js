...
function moveFast() {
    ...
    styleSheet.insertRule('body { filter: blur(10px); }', 0); 
    //styleSheet.insertRule('body { opacity: 0.1; }', 0); 
...
function slowSave() { debounce(function() {
		alert('coucou');
		if(!machineScroll) { 
			saveAmountScrolled();
		}
...
document.getElementById('conte').addEventListener('scroll', slowSave);
